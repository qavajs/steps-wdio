 ...
 */
When('I set {value} {word} storage value as {value}', async function (this: QavajsWdioWorld, storageKey: MemoryValue, storageType: string, value: MemoryValue) {
    await this.wdio.browser.execute<any, any>(function (storageKey: any, storageType: string, value: any) {
 ...
 */
When('I save value of {value} {word} storage as {value}', async function (this: QavajsWdioWorld, storageKey: MemoryValue, storageType: string, key: MemoryValue) {
    const value = await this.wdio.browser.execute<any, [string, string]>(function (storageKey: string, storageType: string) {
        const storage = storageType + 'Storage' as 'localStorage' | 'sessionStorage';
